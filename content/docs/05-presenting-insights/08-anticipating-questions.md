...
> [!NOTE]
> TODO: Insert manuscript screenshot or diagram from the source draft. Source PDF note: `[DIAGRAM Design | Production                       | Sales R&D -> Sourcing -> Manufacture -> QC -> Orders -> Shipping -> Feedback ]`.
...
| All the dimensions | Identify all the information captured by the processes. | Cumulative information diagram. Shows all the information relevant to the business, how they flow, and where they are physically stored. Used to inform what can be reasonably answered. |
| All the relationships | Relate information to their processes in the dimensional model. | Chronological bus. Shows the dimensional model that is implemented. Used to identify what possible questions that can or cannot be answered, particularly any gaps. |
...
This breadth of information requires a variety of techniques to integrate effectively. The technical patterns introduced in earlier chapters are tools the data engineer can use to meet these diverse scenarios. In particular, the inclusion of ID dimensions guarantees that every grain can be recovered when needed. Storytelling dimensions, on the other hand, support queries at the grain of the entity of interest, allowing users to answer questions at that level.
