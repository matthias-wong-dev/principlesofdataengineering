...
> [!NOTE]
> TODO: Insert manuscript screenshot or diagram from the source draft. Source PDF note: `[SCREENSHOT EXAMPLE, Tea, coffee or both]`.
...
Histogram dimensions are used when it is necessary to filter the model by a numeric value. For example, finding the number of helpdesk cases that take more than 3 days to resolve. Measures such as [Total days to process] cannot be used because measures do not work as normal filters in Power BI. Instead, the numeric value needs to be expressed as a dimension.
...
One common theme of this chapter is to push information back from lower-grain than the entity, back to the entity level, and then further abstracted to the dimension. The combination, choice and Sankey dimensions all follow this pattern. They take detail-level complexity and express it as a dimension with a manageable set of values that users can interact with easily.

> [!NOTE]
> TODO: Insert manuscript screenshot or diagram from the source draft. Source PDF note: `[DIAGRAM… Finer grain than entity → Entity → Reference table]`.
